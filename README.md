<p align="center">
<img src="https://raw.githubusercontent.com/TetraTheta/AutoRegionFarm/main/.etc/autoregionfarm_icon.png" alt="AutoRegionFarm Logo">
</p>

# AutoRegionFarm
**AutoRegionFarm protects and replants crops inside selected WorldGuard regions on Paper servers**

![Version](https://img.shields.io/modrinth/v/autoregionfarm?style=for-the-badge&label=Plugin%20Version)
![Game Version](https://img.shields.io/modrinth/game-versions/autoregionfarm?style=for-the-badge&label=Minecraft%20Version)  
[![Modrinth Downloads](https://img.shields.io/modrinth/dt/autoregionfarm?style=for-the-badge&label=Modrinth%20Downloads)](https://modrinth.com/plugin/autoregionfarm)

## Introduction

AutoRegionFarm helps server owners turn selected WorldGuard regions into managed farm areas on Paper servers with WorldEdit and WorldGuard installed. Inside watched regions, immature crops can be protected from accidental breaking, while mature crops can be harvested, delivered to the player, and replanted automatically.

The plugin only acts in WorldGuard regions that you explicitly add with `/arf region add`. This keeps ordinary survival farming unchanged outside managed farm areas and lets each server decide exactly where automatic behavior belongs.

By default, AutoRegionFarm supports wheat, carrots, potatoes, beetroots, and nether wart. Server owners can disable individual crop materials in the configuration, adjust Creative mode behavior for protection and harvesting separately, choose how full inventories are handled, protect farmland from trampling, and allow sneaking players to bypass immature crop protection.

Use `/arf reload` after editing the configuration to apply changes without restarting the server.
...
# Material.matchMaterial() is used, then AutoRegionFarm keeps only supported crop descriptors.
crops:
  materials:
    - BEETROOTS
    - CARROTS
...
# WorldGuard regions where AutoRegionFarm should adjust crop breaking.
# Entries are normalized as "world:region". Commands may add bare region names for a player's current world.
regions:
  watched: []
```

AutoRegionFarm validates recoverable configuration values during startup and reload. Invalid overflow policies fall back to `drop`, invalid notification channels fall back to `action-bar`, unsupported crop materials are removed from `crops.materials`, and watched regions that no longer exist in WorldGuard are removed from the saved list.

## Commands

- `/arf`: Lists watched WorldGuard regions.
- `/arf region`: Lists watched WorldGuard regions.
- `/arf region list`: Lists watched WorldGuard regions.
- `/arf region add <region>`: Adds a WorldGuard region to the watched list.
- `/arf region remove <region>`: Removes a WorldGuard region from the watched list.
- `/arf region delete <region>`: Removes a WorldGuard region from the watched list.
- `/arf region del <region>`: Removes a WorldGuard region from the watched list.
- `/arf reload`: Reloads configuration from disk.
...
AutoRegionFarm listens for crop block breaks at high priority and ignores events already cancelled by other plugins. It checks the broken block against the configured crop registry, then verifies that the block location is inside one of the watched WorldGuard regions.
...
- `autoregionfarm.admin`: Allows managing watched regions and reloading configuration.
